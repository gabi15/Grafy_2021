import sys
from FlowGraph import FlowGraph, generate_random_flow_graph, prepare_multilayer_graph, draw_multilayer_graph,\
    ford_fulkerson
...
def draw_graph_with_flow() -> None:
    ...
    if save_to_file in ["0", "1"]:
        ...
        result_flow_graph = prepare_multilayer_graph(return_g, flow_graph.layers)
        draw_multilayer_graph(result_flow_graph, True, bool(int(save_to_file)), file_name)
    else:
...
def generate_and_draw_random_flow_graph() -> None:
    layers = get_layers()
    save_to_file = input("Do you want to save image to a file? Select an option:\n"
                             "0 - No\n"
                             "1 - Yes\n"
    ...
    if save_to_file in ["0", "1"]:
        ...
        draw_multilayer_graph(graph, False, bool(int(save_to_file)), file_name)
    else:
        print("Wrong input, try again")
        generate_and_draw_random_flow_graph()
